...
if [[ ! -f "$SRV/run.sh" && ! -f "$SRV/.installed" ]]; then
  ...
  # NeoForge server packs (ATM11) DON'T ship a ready-to-run run.sh — they ship
  # the NeoForge installer + startserver.sh. run.sh (plus the libraries/ tree and
  # the @argfiles the systemd unit's `run.sh nogui` depends on) is generated by
  # the installer's --installServer step. Run it once here if run.sh is missing.
  if [[ ! -f "$SRV/run.sh" ]]; then
    installer=$(find "$SRV" -maxdepth 1 -name 'neoforge-*-installer.jar' | head -1 || true)
    [[ -n "$installer" ]] || { log "ERROR: pack has neither run.sh nor a NeoForge installer"; exit 1; }
    log "running NeoForge installer to generate run.sh ($(basename "$installer"))"
